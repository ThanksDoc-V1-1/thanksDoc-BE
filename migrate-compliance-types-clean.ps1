...
# Define all compliance document types to migrate
$complianceTypes = @(
    @{
        key = "medical_license"
...
foreach ($docType in $complianceTypes) {
    try {
        ...
        $response = Invoke-WebRequest -Uri "$productionBaseUrl/compliance-document-types" -Method POST -ContentType "application/json" -Body $jsonPayload
...
if ($errorCount -eq 0) {
    Write-Host "All compliance document types migrated successfully!" -ForegroundColor Green
} else {
