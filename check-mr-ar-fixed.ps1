...
try {
    ...
    if ($response.success -and $response.data.documents) {
        ...
        for ($i = 0; $i -lt [Math]::Min(3, $documents.Count); $i++) {
            ...
            if ($doc.complianceDocumentType) {
                Write-Host "     Type ID: $($doc.complianceDocumentType.id)"
                Write-Host "     Type Name: $($doc.complianceDocumentType.name)"
            }
            Write-Host ""
        }
        ...
        if ($updateResponse.success) {
            ...
            if ($result.verificationCheck) {
                ...
                if ($check.expiredDocuments.Count -gt 0) {
                    Write-Host "   Expired docs: $($check.expiredDocuments -join ', ')" -ForegroundColor Red
                }
            }
        } else {
